...
logger = logging.getLogger("workers.person_cpu")
STOP = False
...
def run_worker() -> None:
    ...
    capability = CapabilityRegistry.make("person", "cpu", ["queue:person:cpu"], batch_capacity=1)

    while not STOP:
        try:
            registry.register(capability)
            message = redis_client.brpop("queue:person:cpu", timeout=2)
        ...
        try:
            trace.emit(
                ...
                component_name="workers.person_cpu",
            )
