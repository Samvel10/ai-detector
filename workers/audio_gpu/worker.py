...
logger = logging.getLogger("workers.audio_gpu")
STOP = False
...
def _consume_batch(redis_client: Redis, queue_name: str, max_batch: int = 4) -> list[dict[str, Any]]:
...
def run_worker() -> None:
    ...
    capability = CapabilityRegistry.make("audio", "gpu", ["queue:audio:gpu"], batch_capacity=4)
    ...
    while not STOP:
        try:
            ...
            time.sleep(1.0)
            continue
        batch = _consume_batch(redis_client, "queue:audio:gpu", max_batch=4)
        if not batch:
            time.sleep(0.2)
            continue
        for item in batch:
            ...
            try:
                trace.emit(
                    ...
                    component_name="workers.audio_gpu",
                )
                process_audio_task(item["task_id"])
            finally:
